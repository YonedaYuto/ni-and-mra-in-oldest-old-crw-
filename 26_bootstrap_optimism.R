...
LABELS_PATH <- here::here("01_labels.R")
if (!exists("relabel_vars")) {
  if (!file.exists(LABELS_PATH)) stop("01_labels.R not found: ", LABELS_PATH)
  source(LABELS_PATH)
}
...
read_fixed_correction <- function() {
  ...
  if (is.null(perf)) {
    message("Section 24 summary (data/bnb_performance_summary.rds) not found; ",
            "the formula-fixed correction cannot be shown. Run Section 24 first.")
    return(NULL)
  }
  ...
  if (!length(auc_col)) {
    message("AUC_corrected is absent from bnb_performance_summary.rds ",
            "(Section 24 was probably run with DO_BOOTSTRAP = FALSE).")
    return(NULL)
  }
  out <- pf[, .(pipeline,
  ...
  if (all(!is.finite(out$AUC_corrected_fixed)))
    message("AUC_corrected in Section 24 is all NA (DO_BOOTSTRAP = FALSE?); ",
...
if (!nrow(summ))
  stop("No pipeline produced an optimism estimate. Run Sections 19, 20, 21 and 14 first.")
...
message("---- Optimism correction: formula fixed (Section 24) vs selection included (Section 26) ----")
print(summ[, .(pipeline,
...
message("AUC_fixed      = Section 24, model formula held fixed (read from ",
...
if (nrow(summ)) {
  ...
  if (all(is.na(pl[kind == "Corrected (formula fixed)", AUC])))
    warning("The formula-fixed correction is unavailable; the figure will show ",
            "two bars per pipeline. Run Section 24 with DO_BOOTSTRAP = TRUE.")
